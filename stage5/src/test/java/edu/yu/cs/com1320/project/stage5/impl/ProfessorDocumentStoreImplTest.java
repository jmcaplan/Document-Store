...
public class ProfessorDocumentStoreImplTest {
    ...
    @Test
    public void testDeleteDoc() throws URISyntaxException{
        DocumentStore store = new DocumentStoreImpl();
        URI uri = new URI("testDeleteDocyu.edu/docs/doc1");
        store.putDocument(new ByteArrayInputStream(this.pdfData1),uri, DocumentStore.DocumentFormat.PDF);
        store.deleteDocument(this.uri1);
        assertEquals("calling get on URI from which doc was deleted should've returned null", null, store.getDocumentAsPdf(this.uri1));
    }
    
    /* 
    // this one passes as of thurs afternoon, problem must be that deleteDocument left it on disk
    @Test
    public void testDeleteDoc(){
        ...
        DocumentStoreImpl storeImpl = (DocumentStoreImpl) store; //added
        assertTrue(storeImpl.deleteDocument(this.uri1)); // used to not be assertion test
        assertTrue(storeImpl.getDocument(this.uri1) == null);   // added
        assertTrue(storeImpl.btree.get(this.uri1) != null); // it's on the disk after a delete!!!!?!
        storeImpl.btree.get(storeImpl.btree.root, this.uri1, storeImpl.btree.height).val = null;
        	// ^^ deletes from memory, and the get removed it from disk
        assertTrue(storeImpl.btree.get(this.uri1) == null); // should be off disk
        assertEquals("calling get on URI from which doc was deleted should've returned null", null, storeImpl.getDocumentAsPdf(this.uri1));
        // summary of error - when a doc is deleted, it is still on disk, and 
        // this for some reason causes updateLastUsedTime to have NPE when it
        // tries to reheapify on line 21
    }
    */

    @Test
    public void testDeleteDocReturnValue() throws URISyntaxException{
        DocumentStore store = new DocumentStoreImpl();
        URI uri = new URI("testDeleteDocReturnValueyu.edu/docs/doc1");
        store.putDocument(new ByteArrayInputStream(this.pdfData1),uri, DocumentStore.DocumentFormat.PDF);
        //should return true when deleting a document
        assertEquals("failed to return true when deleting a document",true,store.deleteDocument(uri));
        //should return false if I try to delete the same doc again
        assertEquals("failed to return false when trying to delete that which was already deleted",false,store.deleteDocument(uri));
