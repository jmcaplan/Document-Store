...
public class HashTableImpl<Key,Value> implements HashTable<Key,Value> {
    ...
    public Value put(Key k, Value v) {
    	...
    	while (currentEntry != null) {
		  ...
		  // there and return old value, and don't add to nElements
		array[index].add(new Entry<Key,Value>(k,v));
		nElements++;
		return null; // last case, if key not present, DO increment nElements
    }
    ...
    private Value delete(Key k) {
    	...
    	Entry<Key,Value> previousEntry = null;
    	if (currentEntry.getKey().equals(k)) {
    ...
    private void doubleArrayIfNecessary() {
    	if ((this.nElements + 1)/4 < ARRAY_LENGTH) {
    		return;
    	} // doubling not necessary
    	else {
    		int oldArrayLength = ARRAY_LENGTH;
    		EntryList<Key,Value>[] oldArray = this.array; // save current contents for copying over
    		this.ARRAY_LENGTH *= 2;
    		this.array = (EntryList<Key,Value>[]) new EntryList[this.ARRAY_LENGTH]; // make new empty container
    		this.nElements = 0; // reflect the emptiness in our instance variable
    		for (int i = 0; i < oldArrayLength;i++) {
    			if (oldArray[i].getFirst() == null) {
    				continue;
    			}
    			else {
    				Entry<Key,Value> entry = oldArray[i].getFirst();
        			while (entry != null) {
